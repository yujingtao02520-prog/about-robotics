"""Train the MLP state-to-action baseline."""
...
def main() -> None:
    ...
    output_path = resolve_path(args.output) if args.output else config_path(config, "paths", "mlp_checkpoint")
    ...
    model = MLPPolicy(state_dim=dataset.state_dim, action_dim=dataset.action_dim)
    ...
    print(f"MLP checkpoint saved to: {checkpoint}")
